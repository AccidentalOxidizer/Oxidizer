...
module.exports = function(sequelize, userid){
  ...
  .then(function(user){
    ...
    var queryString = 'SELECT DISTINCT Comments.id, Comments.UserId, Comments.text, Comments.UrlId, ' + 
      ...
      '(SELECT COUNT(1) AS other FROM Hearts AS h WHERE Comments.id = h.CommentId AND h.UserId = 1) AS HeartedByUser, ' +
      '(SELECT COUNT(1) AS other FROM Flags AS f WHERE Comments.id = f.CommentId AND f.UserId = 1) AS FlaggedByUser, ' +
      '(SELECT COUNT(1) AS other FROM Comments AS c WHERE Comments.id = c.RepliesToId AND c.UserId = 1) AS RepliesToUser, ' +
